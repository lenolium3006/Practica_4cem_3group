...
def sanitize_text(text: str | None) -> str:
    if not text:
        return ""
    if re.search(r"[\w.+-]+@[\w-]+\.[\w.-]+", text) or re.search(r"\+?\d[\d\s().-]{7,}", text):
        return "[sensitive]"
    return text[:500]
...
def log_user_action(message: telebot.types.Message, status: str) -> None:
    ...
    media_type = next(
        (ct for ct in ("photo", "document", "video", "audio", "voice", "sticker", "animation", "location", "contact")
    ...
    with get_db_connection() as conn:
        conn.execute(
            """
            INSERT INTO user_actions (timestamp, user_id, username, chat_id, query_text, status)
            VALUES (?, ?, ?, ?, ?, ?)
            """,
            (
                timestamp,
...
def start_handler(message: telebot.types.Message) -> None:
    ...
    send_safe_reply(
        ...
        (
            ...
            "Доступны сценарии: поиск книги, режим работы, продление, а также тестовый echo-режим."
        ),
    )
...
@bot.message_handler(
    ...
    content_types=["text", "audio", "document", "photo", "sticker", "video", "voice", "location", "contact", "animation"],
)
